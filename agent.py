...
class Agent():
    ...
    def train_policy_net(self, frame):
        ...
        q_next = self.policy_net(next_states[musk]).detach()

        # Find maximum Q-value of action at next state from policy net
        next_state_values = torch.zeros(batch_size, device=device)
        next_state_values[musk] = q_next.max(1)[0]

        # Compute the Huber Loss
        target = next_state_values*self.discount_factor + rewards
        loss = F.smooth_l1_loss(q_current, target.unsqueeze(1))
